...
//-- Setup the Remote Control's UI canvas
  LRemote.setName("Andre-Searching04");
...
//-- Initialize GPIO
  pickup_servo.attach(PIN_SERVO);  // attaches the servo on pin 9 to the servo object
}

void pickup() {
  pickup_servo.write(175);
  delay(1000);
  pickup_servo.write(150);
}
...
void motor_ISR () {
 if (millis() - start_time >= 1000){   /* 每秒更新 */
   // 計算 rpm 時，停止計數
   detachInterrupt( PIN_EINT0 );
   detachInterrupt( PIN_EINT1 );
   stop_time = millis();
...
//-- 輸出至Console
   ...
   */
   ...
   left_count = 0;
   right_count = 0;
   attachInterrupt( PIN_EINT0, left_ISR_counter,  FALLING);
   attachInterrupt( PIN_EINT1, right_ISR_counter, FALLING);
   start_time = millis();
 }
}
...
//labelDright.updateText(String(diffRight));
  ...
  if ( !obj_got && !obj_available && (curY > 150) && (curX > 50) && (curX < 270) ) { //&& (diffUp < 0) 
    obj_got = true;
    beep(a, 500);    
    beep(c, 500);
