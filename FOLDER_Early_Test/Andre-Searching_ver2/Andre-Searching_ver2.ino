...
//-- rpm around 130 when pwm 200
  ...
  if ( rpm_left_fw == 0 && rpm_left_bw == 0 ) {        //-- 停止
    cur_dir_left = 1;
    dif_rpm_left = 0;
  } else if ( cur_dir_left == 1 && rpm_left_bw > 0 ) {  //-- 往前變往後
    cur_dir_left = 0;
    dif_rpm_left = rpm_left_bw;
  } else if ( cur_dir_left == 0 && rpm_left_fw > 0 ) {  //-- 往後變往前
    cur_dir_left = 1;
    dif_rpm_left = rpm_left_fw;
  } else if ( cur_dir_left == 1 && rpm_left_bw == 0 ) { //-- 往前不變
    cur_dir_left = 1;
    dif_rpm_left = rpm_left_fw - cur_rpm_left;
  } else if ( cur_dir_left == 0 && rpm_left_fw == 0 ) { //-- 往後不變
    cur_dir_left = 0;
    dif_rpm_left = rpm_left_bw - cur_rpm_left;
  }

  if ( rpm_right_fw == 0 && rpm_right_bw == 0 ) {         //-- 停止
    cur_dir_right = 1;
    dif_rpm_right = 0;
  } else if ( cur_dir_right == 1 && rpm_right_bw > 0 ) {  //-- 往前變往後
    cur_dir_right = 0;
    dif_rpm_right = rpm_right_bw;
  } else if ( cur_dir_right == 0 && rpm_right_fw > 0 ) {  //-- 往後變往前
    cur_dir_right = 1;
    dif_rpm_right = rpm_right_fw;
  } else if ( cur_dir_right == 1 && rpm_right_bw == 0 ) { //-- 往前不變
    cur_dir_right = 1;
    dif_rpm_right = rpm_right_fw - cur_rpm_right;
  } else if ( cur_dir_right == 0 && rpm_right_fw == 0 ) { //-- 往後不變
    cur_dir_right = 0;
