...
void setup()
{
  LRemote.setName("Linkit Car");        //設定名稱
  ...
  LRemote.setGrid(3, 5);                //設定APP中的顯示格線
    ...
    slider1.setValueRange(0, 250, 0);  //設定滑桿範圍
...
void loop() {
  ...
  if (F.getValue() == 1) {   //如果按了F按鈕
    //if (slider1.isValueChanged()) {        //--如果滑桿數值改變
      analogWrite(10, 0);   //PIN10訊號0
      analogWrite(12, slider1.getValue());   //PIN12訊號為滑桿的數值
      analogWrite(13, 0);   //PIN13訊號0
      analogWrite(17, slider1.getValue());   //PIN17訊號為滑桿的數值
  ...
  if (B.getValue() == 1) {   //如果按了B按鈕
    //if (slider1.isValueChanged()) {    //如果滑桿數值改變
      analogWrite(10, slider1.getValue());   //PIN10訊號為滑桿的數值
      analogWrite(12, 0);   //PIN12訊號0
      analogWrite(13, slider1.getValue());   //PIN13訊號為滑桿的數值
      analogWrite(17, 0);   //PIN17訊號0
