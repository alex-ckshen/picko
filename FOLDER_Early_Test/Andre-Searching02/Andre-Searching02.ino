...
void object_searching () {
  direct_motor( 130, 0, 130, 0);
  delay (200);
  direct_motor(   0, 0, 160, 0);
  delay (200);
  motor_break ();
  delay (2000);
}
...
void object_tracking ( int diffWidth, int diffXoffset ) {
...
int rFast;
  ...
  if ( diffWidth <= MAX_WIDTH*(-1) ) {
    ...
    direct_motor( 0, 110+rKick+rSlow, 0, 130+20);
  
  } else if ( (diffWidth > MAX_WIDTH*(-1)) && (diffWidth <= MIN_WIDTH*(-1)) ) {
    if ( motor_forward ) forward_break();
    direct_motor( 0, 110+rKick, 0, 130);

  } else if ( (diffWidth > MIN_WIDTH*(-1)) && (diffWidth < MIN_WIDTH) ) {
    motor_break();

  } else if ( diffWidth >=  MIN_WIDTH ) {
    if ( diffXoffset >= MAX_OFFSET ) {
      direct_motor( 110+rKick, 0, 130+20+20, 0);

    } else if ( (diffXoffset < MAX_OFFSET) && (diffXoffset >= MIN_OFFSET) ) {
      direct_motor( 110+rKick, 0, 130+20, 0);

    } else if ( (diffXoffset <= MIN_OFFSET*(-1)) && (diffXoffset > MAX_OFFSET*(-1)) ) {
      direct_motor( 110+rKick+rSlow, 0, 130, 0);
      
    } else if ( diffXoffset <= MAX_OFFSET*(-1)) {
      direct_motor( 110+rKick+rSlow+rFast, 0, 130, 0);
      
    } else {
      direct_motor( 110+rKick, 0, 130, 0);
...
void tracking_message ( int diffWidth, int diffXoffset ) {
...
int diffUp;
  ...
  if ( !button_user && digitalRead(BUTTON_PIN) == HIGH )  {
    beep(a, 500);
    button_user = true;
    if ( button_cntr > 1 ) {
      button_cntr = 0;
  ...
  } else if ( button_user && digitalRead(BUTTON_PIN) == HIGH ) {
    beep(c, 500);
    button_user = false;
    if ( button_cntr > 1 ) {
      button_cntr = 0;
...
//-- tracking    
    diffWidth  = (sliderWidth.getValue() - result.width);     //> 0 forwarding
    diffXoffset = (result.xCenter - sliderXoffset.getValue());  //> 0 turn right
    labelDoffset.updateText( String(diffXoffset) );
...
//  labelDright.updateText(String(diffRight));
  ...
  if ( obj_found && !obj_available && (diffUp < 0) && (curY > 150) && (curX > 50) && (curX < 270) ) {
    obj_got = true;
    beep(a, 500);
  } else {
    obj_got = false; 
...
//-- searching 
    if ( obj_available ) {
      tracking_message ( diffWidth, diffXoffset );
    } else {      
      if ( diffRight > 0 && diffUp > 0 ) {
...
//-- Action
  if ( switchStart.getValue() || button_user ) {
    if ( (!switchSearch.getValue() && !switchTrack.getValue()) && !button_user ) {
      checkAndUpdateMotor();
    } else if ( ((switchSearch.getValue() && switchTrack.getValue()) || button_user ) && (!obj_available && obj_found) ) { //-- retry
//    beep(f, 200);
      object_tracking( diffUp*10, diffRight*10 );
      delay(500);
    } else if ( (switchSearch.getValue() || button_user) && (!obj_available && obj_found) ) {
//    beep(cH, 200);
      object_searching ();
    } else if ( (switchTrack.getValue() || button_user) && (obj_available && obj_found) ) {
      object_tracking( diffWidth, diffXoffset );
    } else {
      motor_break();
    } 
  } else {
