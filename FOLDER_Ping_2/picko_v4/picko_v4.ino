...
boolean  func_rpm_sel;
boolean  func_rpm_on;
boolean  func_circle_sel;
boolean  func_circle_on;
boolean  func_track_sel;
boolean  func_track_on;
boolean  func_pickup_sel;
boolean  func_pickup_on;
boolean  func_search_sel;
boolean  func_search_on;
...
#define HUSKY_PICKUP  6
#define HUSKY_LOST    7
#define HUSKY_NOSHOW  8

HUSKYLENS huskylens;
void      printResult( HUSKYLENSResult result );
int       husky_state;
int       pre_husky_state;
const int expect_width   = 60; //max:60
...
//-- initialize the variables we're linked to
  ...
  func_rpm_sel = false;
  func_rpm_on = false;
  func_circle_sel = false;
  func_circle_on = false;
  func_track_sel = false;
  func_track_on = false;
  func_pickup_sel = false;
  func_pickup_on = false;
  func_search_sel = false;
  func_search_on = false;
...
void object_tracking( int diffXcenter, int diffWidth ) {
   ...
   pre_diffWidth   = diffWidth;
    ...
    if ( (diffWidth > 0) && (diffXcenter >= 20) ) {                        //-- large turn right
      rpm_left_fw  = 70;
      rpm_left_bw  = 0; 
      rpm_right_fw = 30; 
      rpm_right_bw = 0;
      ...
      rpm_right_bw = 0;
    } else if ( (diffWidth > 0) && (diffXcenter <= -20) ) {                      //-- large turn left
      rpm_left_fw  = 30;
      rpm_left_bw  = 0; 
      rpm_right_fw = 70; 
      rpm_right_bw = 0;
...
//-- rpm
  ...
  if ( !huskylens.request() ) {
    ...
    if ( husky_state == HUSKY_AVAIL ) { //-- 之前 availavle, 但husky_state尚未更新
      if ( (curY < 240) && (curY > 180) && (curX > 120) && (curX < 200) ) {
        husky_state = HUSKY_PICKUP;
#ifdef PRT_HUSKY
        if ( husky_state != HUSKY_PICKUP ) { 
          Serial.println(F("Object pickup!"));
        }
#endif
      } else {
#ifdef PRT_HUSKY
        if ( husky_state != HUSKY_LOST ) { 
          Serial.println(F("Object lost!"));
        }
#endif
        husky_state = HUSKY_LOST;
      }
    } else {
#ifdef PRT_HUSKY
      if ( husky_state != HUSKY_NOSHOW ) {
        Serial.println(F("Object no show!"));
      }
#endif
      husky_state = HUSKY_NOSHOW;
    }
    preX = curX;
    preY = curY;
    curX = -1;
...
//-- circling test selection
  if ( func_enable && (func_cntr == FUNC_CIRCLE) ) {
    func_circle_sel = true;
    func_circle_on  = true;
    if ( !func_acted ) {
      func_acted = true;
#ifdef PRT_BTN
      Serial.println(String()+F("func_circle_sel on"));
#endif
    }
  } else if ( !func_enable &&
    ...
    func_acted = true;
#ifdef PRT_BTN
    Serial.println(String()+F("func_circle_sel off"));
#endif
  }
  
//-- tracking test selection
  if ( func_enable && (func_cntr == FUNC_TRACK) ) {
    func_track_sel = true;
    func_track_on  = true;
    if ( !func_acted ) {
      func_acted = true;
#ifdef PRT_BTN
      Serial.println(String()+F("func_track_sel on"));
#endif
    }
  } else if ( !func_enable &&
              ( ( !func_acted && (func_cntr == FUNC_TRACK) ) || (pre_func_cntr == FUNC_TRACK) ) ) {
    func_track_on = false;
    pre_func_cntr = 0;
    func_acted = true;
#ifdef PRT_BTN
    Serial.println(String()+F("func_track_sel off"));
#endif
  }
  
//-- pickup test selection
  if ( func_enable && (func_cntr == FUNC_PICKUP) ) {
    func_pickup_sel = true;
    func_pickup_on  = true;
    if ( !func_acted ) {
      func_acted = true;
#ifdef PRT_BTN
      Serial.println(String()+F("func pick on"));
              ...
              ( ( !func_acted && (func_cntr == FUNC_PICKUP) ) || (pre_func_cntr == FUNC_PICKUP) ) ) {
    func_pickup_on = false;
    pre_func_cntr = 0;
    func_acted = true;
#ifdef PRT_BTN
...
//-- searching
  if ( func_enable && (func_cntr == FUNC_SEARCH) ) {
    func_search_sel = true;
    func_search_on  = true;
    if ( !func_acted ) {
      func_acted = true;
#ifdef PRT_BTN
      Serial.println(String()+F("func_search_sel on"));
#endif
    }
  } else if ( !func_enable &&
    ...
    func_acted = true;
#ifdef PRT_BTN
    Serial.println(String()+F("func_search_sel off"));
#endif
  }

//-- rpm test
  if ( func_rpm_sel && func_rpm_on ) {
    rpm_ctrl_en  = true;
    rpm_left_fw  = 40;
    rpm_left_bw  = 0; 
    rpm_right_fw = 40;
    rpm_right_bw = 0;
#ifdef PRT_BTN
    Serial.println(String()+F("func_rpm_sel = ")+func_rpm_sel+F("func_rpm_on = ")+func_rpm_on);
#endif
  } else if ( func_rpm_sel && !func_rpm_on ) {
    rpm_left_fw  = 0;
    rpm_left_bw  = 0; 
    rpm_right_fw = 0; 
    rpm_right_bw = 0;
#ifdef PRT_BTN
    Serial.println(String()+F("func_rpm_sel = ")+func_rpm_sel+F("func_rpm_off = ")+func_rpm_on);
#endif
  }
  
//-- circle test
  if ( func_circle_on ) {
    if ( (husky_state != HUSKY_AVAIL) && (husky_state != HUSKY_PICKUP) ) {
      rpm_left_fw  = 60;
      rpm_left_bw  = 0; 
      rpm_right_fw = 30; 
      rpm_right_bw = 0;
  ...
  } else if ( func_circle_sel && !func_circle_on ) {
    rpm_left_fw  = 0;
    rpm_left_bw  = 0; 
    rpm_right_fw = 0; 
    rpm_right_bw = 0;
...
//-- track test
  if ( func_track_on ) {
    if ( husky_state == HUSKY_PICKUP ) {
      ...
      delay(100);
      object_tracking( 0, -1 );
    } else if ( husky_state == HUSKY_AVAIL ) {
  ...
  } else if ( func_track_sel && !func_track_on ) {
    rpm_left_fw  = 0;
    rpm_left_bw  = 0; 
    rpm_right_fw = 0; 
    rpm_right_bw = 0;
...
//-- main program to search and pick up the ball
  if ( func_search_on && (pickup_cntr == 0) ) {
    if ( husky_state == HUSKY_PICKUP ) {
      object_tracking( 0, 5 );
      delay(100);
      object_tracking( 0, -1 );
  ...
  } else if ( func_search_sel && !func_search_on ) {
    pickup_cntr  = 0;
    rpm_left_fw  = 0;
    rpm_left_bw  = 0; 
    rpm_right_fw = 0; 
...
//-- pwm = 95  steady rpm=60
    ...
    if ( (pre_pwm_left != cur_pwm_left) || (pre_pwm_right != cur_pwm_right) ) {
      Serial.println(String()+F("         sty_rpm_left  = ")+sty_rpm_left
                             +F("; pre_rpm_left  = ")+pre_rpm_left
                             +F("; pre_pwm_left  = ")+pre_pwm_left);
      Serial.println(String()+F("         sty_rpm_right = ")+sty_rpm_right
                             +F("; pre_rpm_right = ")+pre_rpm_right
                             +F("; pre_pwm_right = ")+pre_pwm_right);
      Serial.println(String()+F("         cur_dir_left  = ")+cur_dir_left
                             +F("; cur_rpm_left  = ")+cur_rpm_left
                             +F("; cur_pwm_left  = ")+cur_pwm_left);
      Serial.println(String()+F("         cur_dir_right = ")+cur_dir_right
                             +F("; cur_rpm_right = ")+cur_rpm_right
                             +F("; cur_pwm_right = ")+cur_pwm_right);
    }
#endif
  }
